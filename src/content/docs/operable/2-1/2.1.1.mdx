...
키보드 초점을 받을 수 없는 요소에는 tabindex="0"을 부여하여 초점 이동을 가능하게 하고, keydown 이벤트를 추가하여 키보드 입력([키보드 인터페이스](../../glossary/keyboard-interface))으로도 동일한 기능이 실행될 수 있도록 구현해야 한다. 이때, 키보드 조작 방식은 HTML 기본 요소의 동작 방식을 참고하여 일관되게 적용해야 한다. 

추가로, ARIA 속성을 사용하여 UI의 역할과 상태 정보를 보조 기기에 전달될 수 있게 제공해야 한다.

![이미지. 사용자 정의 스위치 UI 예시](/images/image123.png)
...
![이미지. img 요소로 구현된 버튼 사례](/images/image124.png)
```html
